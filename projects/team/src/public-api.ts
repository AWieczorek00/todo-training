/*
 * Public API Surface of team
 */

export * from './lib/adapters/primary/ui/our-team/our-team.component-module';
export * from './lib/adapters/secondary/infrastructure/firebase-employees.service-module'
export * from './lib/adapters/primary/ui/employee-details/employee-details.component-module'
export * from './lib/adapters/primary/ui/createEmployee/create-employee.component-module'
